...
async def chat(request: Request):
    try:
        ...
        if not message:
            return JSONResponse(status_code=400, content={'error': 'message is required', 'reply': ''})

        # Use long-lived MCP ClientSession started at app startup
        mcp_session = getattr(request.app.state, 'mcp_session', None)
        if mcp_session is None:
            return JSONResponse(status_code=503, content={'error': 'MCP session not initialized', 'reply': ''})
        ...
        if m:
            ...
            try:
                ...
                return JSONResponse(status_code=500, content={'error': str(e), 'reply': f'Error calling read_file: {e}'})
        ...
        if m:
            ...
            try:
                ...
                return JSONResponse(status_code=500, content={'error': str(e), 'reply': f'Error calling get_file_history: {e}'})
        ...
        if m:
            ...
            try:
                ...
                return JSONResponse(status_code=500, content={'error': str(e), 'reply': f'Error calling update_file: {e}'})
        ...
        return JSONResponse(status_code=500, content={'error': str(e), 'reply': f'Internal error: {e}'})
